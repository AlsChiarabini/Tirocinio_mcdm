...
def compute_mcdm_rolling_strategy(df: pd.DataFrame, method_name: str, configuration: dict, weighting: bool = False):
    ...
    types = configuration.get('default_signs', [1]*len(factors))
    ...
    if method_name == 'ahp':
        # Prepara matrice AHP dai pairwise forniti in configurazione
        pairwise = configuration['pairwise']  # dizionario {(f1,f2):valore,...}
        n = len(factors)
        ahp_matrix = np.ones((n, n))
        factor_idx = {f:i for i,f in enumerate(factors)}
        for (f1, f2), val in pairwise.items():
            i, j = factor_idx[f1], factor_idx[f2]
            ahp_matrix[i, j] = val
            ahp_matrix[j, i] = 1/val
        ahp_model = AHP(matrix=ahp_matrix)
        weights_vector = ahp_model()
    ...
    for date in tqdm(reallocation_dates, desc=f"{method_name.upper()} rolling progress"):
        ...
        if weighting:
            long['wret'] = long['RET_RF'] * df_port['me_lag']
            short['wret'] = short['RET_RF'] * df_port['me_lag']
