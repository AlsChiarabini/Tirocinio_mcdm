...
def select_voters_and_signs(data,
    ...
    # Define the condition (example: elements smaller than 0.5)
    condition = lambda x: x < alpha

    # Find elements in pvals_list that satisfy the condition
...
def portfolio_formation(df, factor,  holding_periods,  num_port,  lag=0,  reverse=False,  remove_outliers=False, outliers=[], inclusive=False):
    '''
    ...
    for port_num in range(1, num_port + 1):
        ...
        # -------------------------
        # 3c) Merge EW & VW turnover info and aggregate
        # -------------------------
        ...
        # Example: compute the mean of EW_turnover and VW_turnover across all dates
        mean_vals = merged[["portfolio", "EW_turnover", "VW_turnover"]].mean()
        
        reallocation_rates.append(mean_vals)
    ...
    for date in data_reallocation:
        ...
        for portf in range(1,num_port+1):
            ...
            weights['weight'] = (weights['me_lag']) / (weights['me_lag']).sum()

            # Apply these weights to each date in the holding period
            vw_df = port_df.merge(weights[['PERMNO', 'weight']], on='PERMNO')
            ...
            portfolios_weighted_returns[portf].append(wret)
...
def profile_rolling(df,
    ...
    default_wvoters=configuration['default_wvoters']
    default_wsigns=configuration['default_wsigns']
    union_voters=list(set(default_voters).union(set(default_wvoters)))
    ...
    for data in data_reallocation:
        ...
        mean_factors=df_rolling.loc[df_rolling['PERMNO'].isin(list_permno_on_reallocation_date)][['PERMNO']+union_voters].groupby('PERMNO').mean().reset_index()
        mask_reallocation_data_and_PERMNO=(df['medate']==data)&(df['PERMNO'].isin(list_permno_on_reallocation_date))
        df.loc[mask_reallocation_data_and_PERMNO,[f'av{f}' for f in union_voters]]=mean_factors[union_voters].values
...
def static_profile_rolling(df,
    ...
    wvoters=configuration['default_wvoters']
    wsigns=configuration['default_wsigns']
    union_voters=list(set(voters).union(set(wvoters)))
...
def compute_stratified_MJ_portfolio_strategy(df:pd.DataFrame,
    ...
    if not configuration['cut']:
        print('you did not specify any market size class: defaulting to micro cap')
        cut='micro'
    else:
    ...
    if fix_signs:
        fns_mjrolling={'rank':static_rank_rolling,
                       'vote':static_vote_rolling,
                       'profile':static_profile_rolling}
        lag_mj=lag
    else:
        fns_mjrolling={'rank':rank_rolling,
                       'vote':vote_rolling,
                       'profile':profile_rolling}
        voting_window=configuration['voting_window']
        lag_mj=lag+voting_window
    ...
    try:
        ...
        print("Only rank, profile, and vote rolling methods are implemented")
    
    if fix_signs and all_voters_not_nan_on_reallocation:
        df=df.loc[mask_factor_nans_at_reallocation_dates]  
    
    if compute=='all' or compute=='mj':      
        portfolios['mj'], _ , \
        portfolios_stock_reallocation['mj'] = portfolio_formation(df,
                                                        'mj', 
                                                        K, 
                                                        num_port, 
                                                        lag_mj,
                                                        ...
                                                        outliers=outliers,
                                                        inclusive=inclusive)
    if compute=='all' or compute=='wmj':   
        _ , weighted_portfolios['wmj'], \
        portfolios_stock_reallocation['wmj'] = portfolio_formation(df,
                                                        'wmj', 
                                                        K, 
                                                        num_port, 
                                                        lag_mj,
...
def compute_long_short_quantile_strategy(df:pd.DataFrame,
                                         configuration:dict):
    method=configuration['method']
    num_port=configuration['num_port']
    
    configuration['weighting']=False

    configuration['method']=method
    long_portfolios, _, _ = compute_MJ_portfolio_strategy(df,configuration)

    configuration['method']='r_'+method
    short_portfolios, _, _ = compute_MJ_portfolio_strategy(df,configuration)

    ls_portfolios={}
    ls_portfolios['mj']=long_portfolios['mj'][[f'port{num_port}']]-short_portfolios['mj'][[f'port{num_port}']]

    configuration['weighting']=True
    ...
    ls_weighted_portfolios['wmj']=long_portfolios['wmj'][[f'port{num_port}']]-short_portfolios['wmj'][[f'port{num_port}']]

    return ls_portfolios, ls_weighted_portfolios
